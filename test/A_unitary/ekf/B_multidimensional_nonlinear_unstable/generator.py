...
ekf = Ekf(4)
P0 = np.eye(4) * 200*200*0  # Grande incertitude initiale
ekf.P = P0.copy()  # Grande incertitude initiale
ekf.c = np.array([dt_u, 0, 30, 10, 5])  # Paramètres du système
ekf.cov_u = np.diag([acc_noise_density**2/dt_u, gyro_noise_density**2/dt_u])  # Covariance du bruit de commande
...
for i in range(len(t)):
    ...
    if t[i]-last_z1 >= dt_z1:
        last_z1 = t[i]
        z1[i] = h1(true_x[i], ekf.c) + np.random.randn()*sensor_noise_density/np.sqrt(dt_z1)
    if t[i]-last_z2 >= dt_z2 :
        last_z2 = t[i]
        z2[i] = h2(true_x[i], ekf.c) + np.random.randn()*sensor_noise_density/np.sqrt(dt_z2)


ekf.x = true_x[0].copy()  # Initialisation de l'état
Ekf_X = np.zeros((len(t), 4))
...
input_file = 'test/A_unitary/ekf/B_multidimensional_nonlinear_unstable/test_run/main.cpp'
...
plt.scatter(ekf.c[3], ekf.c[4], label='Beacon 2', color='blue')
plt.legend()
