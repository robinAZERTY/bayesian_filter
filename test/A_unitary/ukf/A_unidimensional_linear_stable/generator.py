...
ukf = Ukf(1)
ukf.x = np.array([0.0])  # Initialisation de l'état du filtre
P0 = np.eye(1) * 200*200  # Grande incertitude initiale
ukf.P = P0.copy()  # Grande incertitude initiale
ukf.c = np.array([dt_u, 10000, 100, 20])  # Paramètres du système
ukf.cov_u = np.array([[control_noise_power**2/dt_u]])  # Covariance du bruit de commande
...
for i in range(1, len(t)):
    ...
    control_inputs[i] = U
    true_temperatures[i] = f(np.array([true_temperatures[i-1]]), np.array([U]), np.array(ukf.c))  # Évolution de la température réelle
    if  this_t - last_z >= dt_z:
        measured_temperatures[i] = true_temperatures[i] + np.random.normal(0, np.sqrt(R[0,0]))  # Mesure bruitée
        last_z = this_t

# Estimation de la température avec ukf
estimated_temperatures = np.zeros((len(t)))
estimated_temperatures[0] = ukf.x.copy()
state_covariances = np.zeros((len(t), 1, 1))
state_covariances[0] = ukf.P.copy()
# Fx = np.zeros((len(t), 1, 1))
# Fu = np.zeros((len(t), 1, 1))
perturbation = np.random.normal(0, np.sqrt(ukf.cov_u[0,0]), len(t))
control_inputs += perturbation
for i in range(1, len(t)):
    this_t = t[i]
    ukf.predict(f, np.array([control_inputs[i]]))
    # Fx[i] = ukf.Fx.copy()
    # Fu[i] = ukf.Fu.copy()
...
init_conditions = {
    'R': R,
    'ukf.C': ukf.c,
    'ukf.X': np.array(estimated_temperatures[0]).reshape(1),
    'ukf.P': P0,
    'ukf.Cov_U': ukf.cov_u,
}
...
init_filter_content += "    ukf.setMeasurementFunction(h," + str(R.shape[0]) + ");\n"
init_filter_content += "}\n"

data_content += init_filter_content

# Lecture du fichier C++ et remplacement entre balises
input_file = 'test/A_unitary/ukf/A_unidimensional_linear_stable/test_run/main.cpp'
